...
class BillDetailsGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the BillDetails entity")
        ...
        .repeat(2) {
            ...
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "date":"2020-01-01T00:00:00.000Z", "name":"SAMPLE_TEXT", "billNumber":null, "phoneNumber":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_billDetails_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
