...
export class BillDetailsComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.billDetailsService.search({
                query: this.currentSearch,
                page: this.page,
                size: this.itemsPerPage,
                sort: this.sort()
            }).subscribe(
                (res: HttpResponse<BillDetails[]>) => this.onSuccess(res.body, res.headers),
    ...
    trackId(index: number, item: BillDetails) {
        return item.id;
    }
    registerChangeInBillDetails() {
        this.eventSubscriber = this.eventManager.subscribe('billDetailsListModification', (response) => this.reset());
    }
